...
4. Start workers, then API instances. The deployment workflow requires `/health/live` and `/health/ready` to report the exact tested commit in production mode, checks every dependency, and rejects publicly exposed metrics or API docs. Retain `deployment-verification-<commit>` with the release record.
